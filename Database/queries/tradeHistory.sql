            ...
            where tradeThreadId in (select id from tradethreads where active = 1) and LOWER(body) LIKE '%confirmed%' and (redditParentId is not null and redditParentId <> '')),
            ...
            FROM messages
            JOIN users on messages.userId = users.id
            where tradeThreadId in (select id from tradethreads where active = 1) and (redditParentId is null or redditParentId = '')),
            trade_log as (
            ...
            where 
commentingUser = 'Lucky_Cold9500' or taggedUser = 'Lucky_Cold9500';


select * from users where redditid = 'Lucky_Cold9500'
select * from messages where userid = 162119